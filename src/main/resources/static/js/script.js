...
function toast(msg, type='success') {
  ...
  const colors = { success:'var(--teal)', error:'var(--rose)', warning:'var(--amber)' };
  ...
  try {
    ...
    if (_cache.cities.length === 0) {
      ...
      citiesList.innerHTML = `<div class="px-3 py-2 d-flex flex-wrap gap-2">${
        _cache.cities.map(c=>`<span class="badge" style="background:var(--sky-lt);color:var(--sky);font-size:12.5px;font-weight:600;padding:5px 12px;border-radius:20px">${escHtml(c.cityName)}</span>`).join('')
      }</div>`;
    }
    ...
    if (_cache.positions.length === 0) {
      ...
      posList.innerHTML = `<div class="px-3 py-2 d-flex flex-wrap gap-2">${
        _cache.positions.map(p=>`<span class="badge" style="background:#FDEBD0;color:#784212;font-size:12.5px;font-weight:600;padding:5px 12px;border-radius:20px">${escHtml(p.title)}</span>`).join('')
      }</div>`;
    }
...
function updateApiStatus(ok) {
  ...
  b.style.background = ok ? 'var(--teal)' : 'var(--rose)';
}
...
function renderEmployersTable(list) {
  ...
  tbody.innerHTML = list.map(e=>`
    <tr>
      ...
      <td><a href="${escHtml(e.companyWebPage||'#')}" target="_blank" style="color:var(--sky)">${escHtml(e.companyWebPage||'—')}</a></td>
    </tr>`).join('');
}
...
const METHOD_COLORS = { GET:'var(--teal)', POST:'var(--sky)', PUT:'var(--amber)', DELETE:'var(--rose)' };
  ...
  el.innerHTML = `<div class="col-12"><div class="row g-2">${
    ENDPOINTS.map(e=>`
      <div class="col-md-6 col-lg-4">
        <div class="d-flex align-items-start gap-2 p-3" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;cursor:pointer" onclick="fillRequest('${e.method}','${e.path}',${e.body?`'${e.body.replace(/'/g,"\\'")}' `:'null'})">
...
async function sendRawRequest() {
  ...
  try {
    ...
    badge.style.background = r.ok ? 'var(--teal)' : 'var(--rose)';
    out.textContent = JSON.stringify(r.data, null, 2);
  } catch(e) {
    badge.textContent = 'ERR'; badge.style.background = 'var(--rose)';
...
function errRow(cols, msg) {
  return `<tr><td colspan="${cols}"><div class="empty-state"><i class="bi bi-exclamation-triangle" style="color:var(--rose)"></i><p style="color:var(--rose)">${escHtml(msg)}</p></div></td></tr>`;
}
...
function autoRefresh() {
    ...
    if (document.getElementById('page-job-ads').classList.contains('active')) {
        loadAds();
    }

    if (document.getElementById('page-employers').classList.contains('active')) {
        loadEmployers();
    }

    if (document.getElementById('page-seekers').classList.contains('active')) {
        loadSeekers();
    }

    if (document.getElementById('page-job-positions').classList.contains('active')) {
        loadPositions();
    }

    if (document.getElementById('page-cities').classList.contains('active')) {
        loadCities();
    }

    if (document.getElementById('page-hr-employees').classList.contains('active')) {
        loadHrEmployees();
    }
}
